...
class HomeController extends Controller
{
    public static function index()
    {
        ...
        $newItems = ShopItem::where('active', 1)->where("deleted", 0)->where('modification_id', 0);
        if ($shop->new_items == 1) {
        ...
        return view('home', [
            'structure' => Structure::where("path", "/")->where("deleted", 0)->first(),
            'groups' => ShopGroup::where("parent_id", 0)->where('active', 1)->where("deleted", 0)->inRandomOrder()->get(),
            ...
            'Comments' => Comment::where("active", 1)->where("deleted", 0)->where("parent_id", 0)->where("grade", 5)->inRandomOrder()->limit(15)->get(),
        ]);
    }
}
